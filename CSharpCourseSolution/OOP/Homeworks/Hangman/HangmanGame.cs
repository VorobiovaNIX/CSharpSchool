...
namespace OOP.Homeworks.Hangman
{
    public class HangmanGame
    {
        ...
        {
            string[] words = File.ReadAllLines("Homeworks\\Hangman\\word_rus.txt");
        ...
        public string GuessLetter(char letter)
        {
            ...
            else if (triesCounter == allowedMisses) GameStatus = GameStatus.Lost;
            return result;
        ...
        private bool IsWin()
        {
            foreach (var item in openIndexes)
            {
                if (item ==false)
                {
                    return false;
